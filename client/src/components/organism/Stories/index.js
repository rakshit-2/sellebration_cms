...
const Stories=(props)=> {
    ...
    function submitPost()
    {
        ...
        if(topic=="")
        {
            alert("Fill the topic correctly");
            return;
        }
        if(img=="")
        {
            alert("Fill the img correctly");
            return;
        }
        var ele=img.replace('h\\','#')
        var check = ele.split("#");
        Axios.post('http://localhost:5000/stories/post',
        {
            link:link,
            date:date,
            info:info,
    ...
    {
        setLoadingGet(true)
        Axios.get('http://localhost:5000/stories/get',
        {
        }).then((res)=>{
  ...
  return (
    ...
    <div className="page__each__outer">
        ...
        <div className='page__each__content' style={{display:display.post}}>
            ...
            <div className='name-outer'>
                <div className="name-label">
                    Topic{'\u00A0'}<i style={{color:"#EF4B4D",width:"10px",height:"10px"}} class="far fa-address-book"></i>
                </div>
                <input  onChange={(e)=>{setTopic(e.target.value)}}  type="text" placeholder={"Topic"} className="name-field"  required='required'/>
            </div>
            <div className='name-outer'>
                <div className="name-label">
