...
const Investor_Company=(props)=> {
    ...
    function submitPost()
    {
        ...
        Axios.post('http://localhost:5000/investor/post',
        {
            name:name,
            link:link,
            img:check[1],
    ...
    {
        setLoadingGet(true)
        Axios.get('http://localhost:5000/investor/get',
        {
            params:{
                checkVal:"company"
            }
  ...
  return (
    ...
    <div className="page__each__outer">
        ...
        <div className='page__each__content' style={{display:display.post}}>
            ...
            <div className='name-outer'>
                <div className="name-label">
                    Detail{'\u00A0'}<i style={{color:"#EF4B4D",width:"10px",height:"10px"}} class="far fa-address-book"></i>
                </div>
                <textarea  onChange={(e)=>{setDetail(e.target.value)}} style={{height:"8rem"}}type="text" placeholder={" `,` seprated without space pages name"} className="name-field"  required='required'/>
            </div>
            <div className='name-outer'>
                <div className="name-label">
                    Link{'\u00A0'}<i style={{color:"#EF4B4D",width:"10px",height:"10px"}} class="far fa-address-book"></i>
                </div>
                <textarea  onChange={(e)=>{setLink(e.target.value)}} style={{height:"8rem"}}type="text" placeholder={" `,` seprated without space pages link"} className="name-field"  required='required'/>
            </div>
            <div className='name-outer'>
                <div className="name-label">
...
export default Investor_Company;